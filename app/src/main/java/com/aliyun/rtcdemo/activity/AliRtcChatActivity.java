...
//               System.out.print("截图结果" + ret);
//                int ret = mAliRtcEngine.startScreenShare();
//                System.out.print("分享屏幕结果" + ret);
...
//            mAliRtcEngine.publishLocalAudioStream(false);//默认不发布音频流
//            mAliRtcEngine.publishLocalVideoStream(false);//默认不发布视频流
//
//            mAliRtcEngine.setDefaultSubscribeAllRemoteAudioStreams(false);//默认不订阅音频流
//            mAliRtcEngine.setDefaultSubscribeAllRemoteVideoStreams(false);//默认不订阅视频流
    ...
    private void joinChannel() {
        ...
        userInfo.setAppId("mivddx5f"); //应用AppID
        userInfo.setNonce("AK-a5997aef-4a37-4eb6-a66f-3dbddd9dfd61"); //随机码。需要加上前缀AK-


        String [] arr = {"https://rgslb.rtc.aliyuncs.com"};


        userInfo.setGslb(arr);  //服务地址，当前请使用["https://rgslb.rtc.aliyuncs.com"]
        userInfo.setTimestamp(1664609612); //令牌过期时间戳
        userInfo.setToken("d461f818f9c1b5ece96198a60af8b07463247a06776b08435f88dd1ee287191a");     //加入频道Token
        userInfo.setChannelId("666666"); //频道ID
        userInfo.setUserId("123");  //用户ID
